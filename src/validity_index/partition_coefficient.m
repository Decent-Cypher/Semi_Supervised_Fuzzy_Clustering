## The partition coefficient (PC) measures the fuzzy degree of final divided clusters by means of the fuzzy partition matrix, and the larger its value, the better the partition result
%% partition_matrix: A matrix of size k x N, where each column corresponds to the memberships of a data point to each cluster

function PC = partition_coefficient(partition_matrix)
    ...
    PC = (1/N)*(sum(sum(partition_matrix .^ 2)));
